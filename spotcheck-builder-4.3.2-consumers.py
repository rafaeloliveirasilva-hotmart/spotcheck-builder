...
astrobox_query_params = {
    ...
    'team_id': [refund_team_id, payment_team_id, access_team_id, purchase_team_id]
}
...
def get_ticket_audit_file_path(base_path=base_dir):
    ...
    file_name = 'QLT Assurance - Spotcheck - ' + dt_string + ' - Consumers.csv'
...
def create_ticket_audit_csv(astrobox_data, opinionbox_data, file_path):
    ...
    with open(file_path, 'w', newline='', encoding="UTF-8") as csvaudit:
        ...
        all_tickets = []
        assign_analysts = {
            'Refund Team': ['Leonardo', 'André', 'Mariane', 'Talita'],
            'Access Team': ['Leonardo', 'André', 'Mariane', 'Talita'],
            'Payment Team': ['Leonardo', 'André', 'Mariane', 'Talita'],
            'Purchase Team': ['Leonardo', 'André', 'Mariane', 'Talita']
        }
