...
def get_ticket_audit_file_path(base_path=base_dir):
    ...
    file_name = 'QLT Assurance - Spotcheck - ' + dt_string + ' - Bad Rated Tickets - Journey and Operations.csv'
...
def create_ticket_audit_csv(astrobox_data, opinionbox_data, file_path):
    ...
    with open(file_path, 'w', newline='', encoding="UTF-8") as csvaudit:
        fieldnames = ['TICKET ID', 'TICKET CREATED AT', 'AGENT NAME', 'AGENT E-MAIL', 'TEAM', 'CLUSTER', 'CONTACT REASONS', 'TRANSACTION NUMBER', 'PRODUCT ID', 'FIRST RESOLUTION', 'SURVEY SENT AT', 'SURVEY ANSWERED AT', 'P1', 'P2', 'P3', 'LINK TO SPOTCHECK', 'OPEN TICKET IN ZENDESK', 'ASSIGNED TO', 'ANALYSIS FINISHED?', 'COMMENTS']
        ...
        all_tickets = []
        assign_analysts = {
            '[International] Users Team': ['Rafael', 'Brenno', 'Gabriela', 'Joanderson', 'Raffão', 'André', 'Talita', 'Leonardo', 'Mariane'],  
        }
